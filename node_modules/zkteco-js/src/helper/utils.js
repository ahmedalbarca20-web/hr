...
function decodeString(buf) {
    ...
    const cleanBuf = firstNull !== -1 ? buf.slice(0, firstNull) : buf;

    if (cleanBuf.length === 0) return '';

    // Check if it's valid UTF-8. If it contains the replacement char, it's probably not.
    const strUtf8 = cleanBuf.toString('utf8');
    if (!strUtf8.includes('\uFFFD')) {
        return strUtf8.trim();
    }

    // Fallback to CP1256 for Arabic
    try {
        return iconv.decode(cleanBuf, 'win1256').trim();
    } catch (e) {
        return strUtf8.trim();
    }
}
...
module.exports.decodeUserData72 = (userData) => {
    const user = {
        ...
        password: userData
            ...
            .split('\0')
            .shift(),
        name: decodeString(userData.slice(11, 11 + 24)),
