...
class ZTCP {
    ...
    readWithBuffer(reqData, cb = null) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                console.error('[TCP][readWithBuffer] Error:', err.message);
                reject(err);
                return;
            }

            if (!reply) {
                reject(new Error('REPLY_IS_EMPTY'));
                return;
            }
    ...
    async setUser(uid, userid, name, password, role = 0, cardno = 0) {
        try {
            ...
            commandBuffer.write(password.padEnd(8, '\0'), 3, 8, 'ascii');

            // Encode name in CP1256 for Arabic support
            const nameBuf = iconv.encode(name, 'win1256');
            const paddedName = Buffer.alloc(24, 0);
            nameBuf.copy(paddedName);
            paddedName.copy(commandBuffer, 11);

            commandBuffer.writeUInt32LE(parseInt(cardno), 35);
            // Using writeUInt32LE instead of writeUInt16LE at 35 might be needed for large card numbers, 
            // but the original had 35, 4. Wait, original was readUIntLE(35, 4) in decode.

            commandBuffer.write(userid.padEnd(9, '\0'), 48, 9, 'ascii');
