...
let RolesGuard = class RolesGuard {
    ...
    canActivate(context) {
        ...
        if (user.email && typeof user.email === 'string') {
            ...
            if (emailLower === 'superadmin@hr-demo.local' ||
                emailLower === 'opsadmin@hr-demo.local') {
                userRole = 'super_admin';
            }
        }
        if (!userRole || !roles.includes(userRole)) {
            console.warn(`Access denied for ${user.email}. Role: ${userRole}. Required: ${roles.join(', ')}`);
            throw new common_1.ForbiddenException(`Insufficient permissions. Required roles: ${roles.join(', ')} but got ${userRole}`);
        }
        return true;
    }
};
